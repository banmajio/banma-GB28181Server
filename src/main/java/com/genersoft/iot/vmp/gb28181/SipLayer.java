...
public class SipLayer implements SipListener {
	...
	private ThreadPoolExecutor initSipServer() {
		...
		processThreadPool = new ThreadPoolExecutor(processThreadNum, processThreadNum, 0L, TimeUnit.MILLISECONDS,
				processQueue, new ThreadPoolExecutor.CallerRunsPolicy());
	...
	private SipProvider startTcpListener() {
		...
		} catch (TransportNotSupportedException | InvalidArgumentException | TooManyListenersException
	...
	public void processResponse(ResponseEvent evt) {
		...
		if ((status >= 200) && (status < 300)) { // Success!
			...
			logger.warn("接收到失败的response响应！status：" + status + ",message:"
					+ response.getReasonPhrase()/* .getContent().toString() */);
