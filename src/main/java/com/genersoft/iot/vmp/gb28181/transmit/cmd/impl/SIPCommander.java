...
public class SIPCommander implements ISIPCommander {
	...
	public boolean ptzCmd(Device device, String channelId, int leftRight, int upDown, int inOut, int moveSpeed,
		...
		try {
			...
			Request request = headerProvider.createMessageRequest(device, ptzXml.toString(), "ViaPtzBranch",
					"FromPtzTag", null);
	 ...
	 */
	@Override
	public boolean frontEndCmd(Device device, String channelId, int cmdCode, int parameter1, int parameter2,
			int combineCode2) {
		try {
...
//			content.append("o="+channelId+" 0 0 IN IP4 "+mediaInfo.getWanIp()+"\r\n");
			...
			Request request = headerProvider.createInviteRequest(device, channelId, content.toString(), null, "live",
					null, ssrc);
	 ...
	 */
	@Override
	public void playbackStreamCmd(Device device, String channelId, String startTime, String endTime,
		...
		try {
			...
			Request request = headerProvider.createPlaybackInviteRequest(device, channelId, content.toString(), null,
					"playback", null);
	...
	public void streamByeCmd(String streamId, SipSubscribe.Event okEvent) {
		...
		try {
			...
			InfoCseqCache.CSEQCACHE.remove(streamId);
	...
	public boolean deviceInfoQuery(Device device) {
		try {
			...
			Request request = headerProvider.createMessageRequest(device, catalogXml.toString(), "ViaDeviceInfoBranch",
					"FromDeviceInfoTag", null);
	...
	public boolean catalogQuery(Device device, SipSubscribe.Event errorEvent) {
		...
		try {
			...
			Request request = headerProvider.createMessageRequest(device, catalogXml.toString(), "ViaCatalogBranch",
					"FromCatalogTag", null);
	...
	public boolean recordInfoQuery(Device device, String channelId, String startTime, String endTime) {
		...
		try {
			...
			Request request = headerProvider.createMessageRequest(device, recordInfoXml.toString(),
					"ViaRecordInfoBranch", "FromRecordInfoTag", null);
	...
	private ClientTransaction transmitRequest(Device device, Request request, SipSubscribe.Event errorEvent)
			throws SipException {
		return transmitRequest(device, request, errorEvent, null);
	}

	private ClientTransaction transmitRequest(Device device, Request request, SipSubscribe.Event errorEvent,
			SipSubscribe.Event okEvent) throws SipException {
		ClientTransaction clientTransaction = null;
	...
	// banmajio
	public void playPause(Device device, StreamInfo streamInfo) {
		try {
	...
	public void playReplay(Device device, StreamInfo streamInfo) {
		try {
			StringBuffer content = new StringBuffer(200);
	...
	public void playBackSeek(Device device, StreamInfo streamInfo, long seektime) {
		try {
			StringBuffer content = new StringBuffer(200);
			content.append("PLAY RTSP/1.0\r\n");
			content.append("CSeq: " + InfoCseqCache.CSEQCACHE.get(streamInfo.getStreamId()) + "\r\n");
	...
	public void playBackSpeed(Device device, StreamInfo streamInfo, Integer speed) {
		try {
			StringBuffer content = new StringBuffer(200);
			content.append("PLAY RTSP/1.0\r\n");
			content.append("CSeq: " + InfoCseqCache.CSEQCACHE.get(streamInfo.getStreamId()) + "\r\n");
