...
public class ZLMRESTfulUtils {
	...
	@Value("${media.timeout_sec}")
	private String mediaTimeoutSec;

	@Value("${media.expire_sec}")
	private String mediaExpireSec;
	...
	// banmajio
	public JSONObject getSnap(String url) {
		OkHttpClient client = new OkHttpClient();
		String apiUrl = String.format("http://%s:%s/index/api/%s?secret=%s&url=%s&timeout_sec=%s&expire_sec=%s",
				mediaIp, mediaPort, "getSnap", mediaSecret, url, mediaTimeoutSec, mediaExpireSec);
		JSONObject responseJSON = null;
		logger.debug(apiUrl);
